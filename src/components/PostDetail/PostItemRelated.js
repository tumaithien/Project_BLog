...
function PostItemRelated({ post }) {
    const{title, createDate, slug, author, authorId} = post
    const {dateRelative} = formatRelativeDate(createDate)
    return (
        <article className="related-post__card">
            <Link to={slug} className="related-post__title">{title}</Link>
            <div className="related-post__info">
                <Link className="related-post__author" to={genUserLink(authorId)}>{author.nickname}</Link>
